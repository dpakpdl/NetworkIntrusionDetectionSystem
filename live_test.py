...
def classify(load,mac_src,ip_src,addLine):
    global f
    global stopEv
    #finding User-Agent
    try:
        ...
        if start:
            ...
            if dictionary['Method']=='0':
                ...
                try:
                    ...
                    if not stopEv.is_set():
                        if addLine!=None:
                            addLine(mac_src)
                            addLine(ip_src)
                            addLine(str(load[srt:finish]))
                            addLine(urllib.unquote(str(load[start+1:end])))
                            addLine(str(classed))
                        print >>f,mac_src + "\n" + ip_src + "\n" + str(load[srt:finish])
                ...
                if not stopEv.is_set():
                    if addLine!=None:
                        ...
                        addLine(ip_src)
                        addLine(str(load[srt:finish]))
                        addLine(urllib.unquote(str(reg)))
                        addLine(str(classed))
                    print >>f,mac_src + "\n" + ip_src + "\n" + str(load[srt:finish])
        ...
        if IP in packet:
            ...
            for att in methods:
                if att in str(packet):
                    ...
                    classify(packet.load,packet.src,ip_src,g)
