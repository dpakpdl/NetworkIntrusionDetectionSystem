...
def filter(load):
    ...
    for i in lv:
        ...
        if p != -1:
            ...
            if key=='Content-Type':
                ...
                if key == "Host":
                    dictionary[key] = str(random.randint(0, 2))
                else:
    ...
    content_type = host = content_length = method = payload = None
    for k,v in dictionary.items():
        f.flush()
           
        print >> f,k+': '+v
    print >> f
    print (dictionary.get('Content-Type'), dictionary.get('Host'), dictionary.get('Content-Length'), dictionary.get('Method'), dictionary.get('Payload'))
    dictionary.clear()
